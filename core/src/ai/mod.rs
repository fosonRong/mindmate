...
pub const STOCK_TEMPLATES_HISTORY: &[(&str, &[&str])] = &[
    (
        "daily",
        &[
        r#"你是我的个人工作助手。请根据以下今日记录，生成一份简洁的日报。
...
# 要求
1. 输出 Markdown，标题「☀️ 今日简报」
2. 如果上面已有今天记录的内容，先给「今日进展」：用一两句把已记录的事**归纳润色**（不要逐条照抄），再给「今日重点」
3. 昨日遗留、今日重点（按优先级给出 Top3 建议顺序）、一句话鼓励
4. 不超过 250 字，不编造内容"#
        ],
    ),
    (
...
3. 温和、克制，不超过 200 字，不编造内容"#
    ...
    (
        "review",
        &[
        r#"你是我的个人数据分析师。请根据以下本周数据，输出周度复盘洞察。
...
# 输出格式
直接输出 Markdown 正文；不要把整份内容包在 ``` 代码块里。"#;
...
pub const DEFAULT_BRIEF: &str = r#"你是我的个人工作助手。请生成今天的我的简报。
