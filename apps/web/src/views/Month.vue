  ...
  <div class="col-stack">
    <section class="card">
      <div class="row" style="margin-bottom: 10px">
        <div class="card-title" style="font-size: 15px">月度进度</div>
        <span class="card-sub">{{ monthTitle(anchor) }}</span>
        <div class="spacer"></div>
        <span class="small muted">已录 {{ stats?.nodeCount ?? 0 }}/{{ projectGoal }} 条</span>
        ...
        <input
          ...
          @change="pickMonth"
        />
        <button class="btn btn-sm" @click="anchor = todayStr(); load()">本月</button>
      ...
      <div class="small muted" style="margin-top: 8px">
        有录入 {{ stats?.daysWithRecords ?? 0 }}/{{ stats?.totalDays ?? daysInMonth }} 天 ·
        格内显示当日记录摘要与待办标题 · 点击任意日期查看或补录
      </div>
    </section>
    ...
    <section v-if="summary" class="card" :class="{ 'month-end': summary.isMonthEnd }">
      <div class="row" style="margin-bottom: 10px">
        <div class="card-title" style="font-size: 15px">本月小结</div>
        <span class="card-sub">{{ summary.month }}</span>
        <div class="spacer"></div>
        <span v-if="summary.isMonthEnd" class="badge ok">月初至今已收官</span>
      </div>
      <div class="summary-grid">
        <div class="summary-item">
          <div class="num">{{ summary.daysWithRecords }}<span class="unit">/{{ summary.totalDays }} 天</span></div>
          <div class="lbl">本月记录天数</div>
        </div>
        <div class="summary-item">
          <div class="num">{{ summary.doneTodos }}<span class="unit">/{{ summary.totalTodos }} 件</span></div>
          <div class="lbl">完成待办</div>
        </div>
        <div class="summary-item">
          <div class="num">{{ summary.longestStreak }}<span class="unit">天</span></div>
          <div class="lbl">连续最长</div>
        </div>
        <div class="summary-item">
          <div class="num">{{ summary.nodeCount }}<span class="unit">条</span></div>
          <div class="lbl">记录节点 · 日均 {{ summary.avgPerActiveDay }}</div>
        </div>
      </div>
    </section>
      ...
      <aside class="drawer">
        <div class="row" style="margin-bottom: 12px">
          <div>
            ...
            <div class="small muted">{{ weekdayLabel(drawerDate) }} · {{ drawerNodes.length }} 条记录</div>
          </div>
          <div class="spacer"></div>
        ...
        <div style="flex: 1; overflow-y: auto; margin-top: 12px">
          <div v-if="drawerNodes.length === 0" class="empty" style="padding: 30px 0">
            <div class="ill">🕐</div>
            <div class="t">这一天还没有记录</div>
            <div class="d">补录的内容会标记为「补录」</div>
          </div>
          <div v-else class="tl">
            <div v-for="(n, i) in drawerNodes" :key="n.id" class="tl-item" :class="{ backfill: n.isBackfill }">
              ...
              <div class="tl-bubble">
                <div class="tl-meta">
                  ...
                  <span v-if="n.isBackfill" class="small muted">补录</span>
                </div>
          ...
          <div v-if="todos.schedule && (todos.schedule.schedules.length || todos.schedule.todos.length)" style="margin-top: 16px">
            <div class="card-title" style="font-size: 14px; margin-bottom: 8px">该日待办</div>
