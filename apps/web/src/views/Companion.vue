...
<template>
  <div class="col-stack">
    <div class="tabs">
      <button :class="{ on: tab === 'report' }" @click="tab = 'report'">{{ $t('报告') }}</button>
      <button :class="{ on: tab === 'review' }" @click="tab = 'review'">{{ $t('复盘') }}</button>
      <button :class="{ on: tab === 'chat' }" @click="tab = 'chat'">{{ $t('问答') }}</button>
    </div>
    ...
    <section v-if="tab === 'report'" class="card">
      <div class="row wrap" style="margin-bottom: 12px">
        ...
        <router-link to="/settings" class="btn btn-sm">{{ $t('⚙️ 模型配置') }}</router-link>
      ...
      <div class="report-box">
        <div v-if="!reportContent && !reportLoading" class="empty">
          <div class="ill">✨</div>
          <div class="t">{{ $t('报告从这里开始') }}</div>
          <div class="d">{{ $t('基于你录入的节点与待办完成情况自动生成') }}</div>
          <button class="btn btn-primary" style="margin-top: 6px" @click="generate">{{ $t('生成{a}', { a: rtypeLabel[rtype] }) }}</button>
        </div>
        <template v-else>
      ...
      <div v-if="reportContent" class="row wrap" style="margin-top: 12px">
        ...
        <div class="spacer"></div>
        <button class="btn btn-sm" @click="copyReport">{{ $t('📋 复制 MD') }}</button>
        <button class="btn btn-sm" @click="exportReport">{{ $t('⬇️ 导出') }}</button>
      </div>

      <!-- 历史 -->
      <div v-if="history.length" style="margin-top: 16px">
        <div class="card-title" style="font-size: 14px; margin-bottom: 8px">{{ $t('历史{a}', { a: rtypeLabel[rtype] }) }}</div>
        <div class="stack sm">
          <div v-for="h in history.slice(0, 8)" :key="h.id" class="row" style="gap: 8px; font-size: 13px">
            ...
            <span v-if="!h.isAi" class="badge">{{ $t('本地') }}</span>
            <div class="spacer"></div>
    ...
    <section v-if="tab === 'review'" class="card">
      <div class="row" style="margin-bottom: 12px">
        <div class="card-title" style="font-size: 15px">{{ $t('周度智能复盘') }}</div>
        <span v-if="reviewAt" class="card-sub">{{ reviewAt }}</span>
        <div class="spacer"></div>
        <button v-if="reviewLoading" class="btn btn-sm" @click="abortReview?.(); reviewLoading = false">{{ $t('停止') }}</button>
      ...
      <div v-if="!reviewContent && !reviewLoading" class="empty">
        <div class="ill">📊</div>
        <div class="t">{{ $t('还没有复盘') }}</div>
        <div class="d">{{ $t('智伴会分析本周的记录节奏、完成率与拖延规律') }}</div>
      </div>
      ...
      <template v-else>
        ...
        <details :open="!insights.length">
          <summary class="small muted pointer">{{ $t('查看完整复盘内容') }}</summary>
    ...
    <section v-if="tab === 'chat'" class="card" style="display: flex; flex-direction: column; height: calc(100vh - 210px)">
      <div class="row" style="margin-bottom: 10px">
        <div class="card-title" style="font-size: 15px">{{ $t('智伴问答') }}</div>
        <span class="card-sub">{{ $t('基于你自己的本地数据') }}</span>
        <div class="spacer"></div>
        <button class="btn btn-sm" @click="clearChat">{{ $t('清空') }}</button>
      </div>

      <div ref="chatScroll" class="chat-scroll">
        <div v-if="messages.length === 0" class="empty">
          <div class="ill">💬</div>
          <div class="t">{{ $t('问我关于你记录的任何事') }}</div>
          <div class="d">{{ $t('例如：本周完成了几件待办？') }}</div>
      ...
      <div class="quick-entry" style="margin-top: 10px">
        ...
        <div class="tags-row" style="opacity: 1">
          <span class="small muted">{{ $t('Enter 发送 · 数据不出本机') }}</span>
          <div class="spacer"></div>
